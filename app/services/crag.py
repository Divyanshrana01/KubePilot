# CRAG (Corrective RAG): checks if retrieved documents are actually useful for the
# question, and if not, falls back to a live web search instead of answering blindly.
...
def crag_pipeline(
    ...
    """Grade retrieved chunks and swap in web search results if they're not good enough."""

    if not enable_crag:
        # Feature flag off: just pass chunks through untouched.
        return (
            chunks,
            CRAGEvaluation(
                relevance_score=1.0,
    ...
    if not chunks:
        # Nothing came back from retrieval at all, so go straight to web search.
        ...
        try:
            ...
            # No Tavily key configured, so we can't fall back to the web.
