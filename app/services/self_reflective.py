...
# The 0.85 cutoff below mirrors settings.reflection_min_score; keep them in sync or
# the grader's verdict and should_regenerate()'s retry decision will disagree.
...
def reflect_on_answer(
    ...
    try:
        ...
        # Fail open: a broken grader call shouldn't block the answer from reaching the user.
...
def should_regenerate(reflection: ReflectionResult, iteration: int) -> bool:
    # Both the grader's verdict and the score threshold must agree, and retries are capped
    # so a stubbornly low-scoring loop can't regenerate forever.
    return (
