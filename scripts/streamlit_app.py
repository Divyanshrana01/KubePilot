...
def _auth_section(base_url: str) -> None:
    ...
    with tab_reg:
        ...
        if st.button("Create Account", width="stretch", key="btn_register"):
...
def _upload_section(base_url: str) -> None:
    ...
    if st.button("🚀 Upload & Index", width="stretch", key="btn_upload"):
...
def _query_section(base_url: str, lesson_info: dict) -> None:
    ...
    for row_start in range(0, len(preset_items), 4):
        ...
        for idx, (label, cfg) in enumerate(row):
            with cols[idx]:
                ...
                if st.button(label, width="stretch", key=f"preset_{row_start + idx}", help=help_text):
    ...
    # Submit
    if st.button("🚀 Submit Query", width="stretch", key="btn_query"):
...
def _sql_approval_section(base_url: str) -> None:
    ...
    with st.container(border=True):
        ...
        with c_approve:
            if st.button("✅ Approve & Execute", width="stretch", type="primary"):
